...
class QuizManager_2 {
  ...
  Future<void> LoadQuestions(int numberOfQuestions) async {
    var url =
        'https://opentdb.com/api.php?amount=$numberOfQuestions&category=17&difficulty=medium&type=multiple';
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < json['results'].length; i++) {
        ...
        List<Option> options = [];
        options.add(Option(_parseHtmlString(questionJson['correct_answer']), 10));
        for (int j = 0; j < questionJson['incorrect_answers'].length; j++) {
          options.add(Option(_parseHtmlString(questionJson['incorrect_answers'][j]), 0));
        }
        Question question = Question(_parseHtmlString(questionJson['question']), options);
